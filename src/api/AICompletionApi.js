...
export async function getRecipes(userInputText) {
  try {
    ...
    const response = await axios.post(
      configCompletionAIAPI.completionAIApiURL,
      {
        messages: [
          {
            role: 'user',
            content: `I will ask you a question that, if the response is positive, you may respond only the text of one recipe that contains ${userInputText} as ingredient.
            If response is negative, return only with the this phrase: Error: As far as I know, it is not human food. Maybe eating ${userInputText} won't do you any good.
            This is the question: Is there only the name of a human food in the following text: ${userInputText}`,
          },
        ],
    ...
    if (response.status === 200 && !recipe.recipe.startsWith('Error')) {
      ...
      // below condition define what image AI API will be called
